...
class details():
    def validate_details(self,tno,dt,src,dst,tkts):
        self.tno=tno
        self.dt=dt
        self.src=src
        self.dst=dst
        ...
        db = cl['hcl_data']
        col = db['tkt_book']
        col.insert_one({"trin no:":self.tno,"date:":self.dt,"source":self.src,"destination":self.dst,"Tickets":self.tkts})
    def details_func(self):
        ...
        Button(base, text='Submit', width=10, bg='brown', fg='white',command=lambda :self.validate_details(self.tno.get(),self.dt.get(),self.src.get(),self.dst.get(),self.tkts.get())).place(x=120, y=280)
        base.mainloop()
